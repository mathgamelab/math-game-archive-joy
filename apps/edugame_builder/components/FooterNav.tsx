...
const FooterNav: React.FC<FooterNavProps> = ({ currentStep, onNext, onPrev }) => {
  return (
    <>
      ...
      <div className="hidden md:block fixed right-8 top-1/2 -translate-y-1/2 z-20">
        <button 
          onClick={onNext}
          className="w-14 h-14 rounded-full bg-gradient-to-br from-orange-500 to-yellow-500 shadow-xl flex items-center justify-center text-white hover:from-orange-600 hover:to-yellow-600 transition-all hover:scale-105"
        >
      ...
      <div className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-6 z-40">
        ...
        <button 
          onClick={onNext}
          className="w-16 h-16 rounded-full bg-gradient-to-br from-orange-500 to-yellow-500 shadow-2xl flex items-center justify-center text-white"
        >
